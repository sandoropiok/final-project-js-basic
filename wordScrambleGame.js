...
const words = {
  ...
  hard: ["javascript", "asynchronous", "algorithm", "framework", "randomization"]
};
...
function getNextDifficulty() {
  if (correctAnswers >= getWordsToSolve(currentDifficulty) && currentDifficulty === 'easy') {
    console.log(chalk.yellow("Great job! Moving to Medium level."));
    correctAnswers = 0;
    return 'medium';
  } else if (correctAnswers >= getWordsToSolve(currentDifficulty) && currentDifficulty === 'medium') {
    console.log(chalk.red("Impressive! Moving to Hard level."));
    correctAnswers = 0;
    return 'hard';
  } else if (correctAnswers >= getWordsToSolve(currentDifficulty) && currentDifficulty === 'hard') {
...
function playRound() {
  ...
  while (attempts > 0) {
    const guess = readline.question("Guess the word (or type 'hint' for help): ").toLowerCase();
    ...
    if (guess === randomWord) {
      ...
      console.log(chalk.yellow(`Hint: The word starts with '${randomWord[0].toUpperCase()}' and ends with '${randomWord[randomWord.length - 1].toUpperCase()}'`));
    ...
    if (attempts === 0) {
      console.log(chalk.red(`Out of attempts! The word was: ${randomWord.toUpperCase()}`));
